...
const navLinks = [
  ...
  {
    title: "Events",
    // MODIFICATION HERE: Change the path to explicitly include '/all'
    // which is the desired default sub-route in the EventScreen's Routes setup.
...
function DashboardPanel({ onNavChange, activeNav: propActiveNav }) {
  ...
  useEffect(() => {
    ...
    if (pathname.toLowerCase().includes("/dashboard/venues")) {
      ...
      // Keep checking for just '/dashboard/events' for the purpose of highlighting 'Events'
      newActiveNav = "Events";
    }
