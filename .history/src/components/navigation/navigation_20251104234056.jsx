...
function Navigation({onHomeClick,onSolutionsClick, onAboutUsClick, onHowWeWorkClick }) {
  ...
  // --- CORRECTED Style for active link border ---
  const getActiveLinkStyle = ({ isActive }) => ({
    color: isActive ? "#A04D07" : "#737373",
    // ONLY apply the border bottom style when isActive is true
    borderBottom: isActive ? "2px solid #A04D07" : "none", 
    // Ensure padding is always consistent to prevent shifting
    paddingBottom: "2px", 
  });
  // -----------------------------------------------------------
  
  // Note on one-page scrolling: The 'isActive' property only works reliably 
  // for route-based links (like Home "/") or links using 'end'. 
  // For section scrolling (onClick handlers), you'll need to use a separate 
  // 'active' state or prop passed down to this component to style them correctly. 
  // The current logic handles the 'Home' route ('/') which uses 'isActive'.
    ...
    <div className="d-flex fixed-top bg-ff container-adjustment justify-content-between align-items-center">
      ...
      <div className="d-flex align-items-center w-50 justify-content-between d-lg-flex d-none">
        <NavLink
          ...
          // Use the corrected active style function
          style={getActiveLinkStyle} 
          className="text-decoration-none txt-73 fw-600 fs-16"
          end // 'end' ensures 'Home' is only active on the exact path '/'
        >
          Home
        </NavLink>
        <NavLink
