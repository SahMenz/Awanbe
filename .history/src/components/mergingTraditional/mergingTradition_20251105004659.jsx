...
// A utility component for staggered text animation (Word-by-word)
const AnimatedText = ({ text, className }) => {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      transition={{ staggerChildren: 0.08 }} // Stagger the animation of individual words
    >
      {words.map((word, index) => (
        <motion.span
          key={index}
          variants={wordVariants}
          // The whitespace is preserved by adding a non-breaking space after each word
          // to prevent all words from collapsing onto a single line during animation.
...
function MergingTradition() {
  ...
  // useInView hook to detect when the element is visible in the viewport
  // The 'amount: 0.2' means the animation triggers when 20% of the component is visible.
  const isInView = useInView(ref, { once: true, amount: 0.2 });

  // Common variants for the section content to slide up and fade in
  const itemVariants = {
  ...
  return (
    <div className="mt-4 container-adjustment" ref={ref}>
      <div>
        ...
        <div className="d-flex align-items-center justify-content-between flex-wrap flex-lg-nowrap">
          ...
          <div className="fw-500 fs-43 col-12 col-lg-5">
            <motion.div
              ...
              transition={{ staggerChildren: 0.1 }} // Stagger lines of the headline
            >
              {/* Using a regular motion.div for the headline for simplicity in this structure */}
              <motion.span variants={itemVariants} className="d-block">
          ...
          <motion.div
            ...
            initial="hidden"
            animate={isInView ? "visible" : "hidden"}
            transition={{ duration: 0.7, delay: 0.3, ease: "easeOut" }} // Add a slight delay
          >
        ...
        {/* BOTTOM SECTION: Challenge and Response Cards */}
        <motion.div
          ...
          initial="hidden"
          animate={isInView ? "visible" : "hidden"}
          transition={{ staggerChildren: 0.3, delay: 0.5 }} // Stagger the two main cards
        >
          {/* Challenge Card */}
          <motion.div
